...
@Slf4j
public class DDTestController01 {

    @Value("${server.synchronized}")
...
//            resultMap.put("sound_file_dir", "/ddt/fs/sounds/cti/acd");
                ...
                String question = (String) tokenMap.get("question");
                ...
                if ("complete".equals(input_type)) {
                    if (input_args.contains("hangup")) {
                        ...
                        resultMap.put("privatedata", "test");
                        resultMap.put("playbacks", Collections.singletonList("您好，请问您有什么问题呢？可以向我提问！"));
                        resultMap.put("quickresponse", true);
                    ...
                    if ("S".equals(prefix)) {
                        ...
                        if (text.contains("挂断")) {
                            ...
                            checkedQuestion(play_progress,text,resultMap,asrAddr,appid);
                        }
                    }
                    if ("D".equals(prefix)) {
                        resultMap.put("action", "cti_play_and_detect_speech");
...
//        }
    ...
    private void checkedQuestion(Integer play_progress,String text,Map<String, Object> resultMap,String asrAddr,String appid){
        if(0<play_progress&&text.length()>3){
            log.info("提问 ===>{}", text);
